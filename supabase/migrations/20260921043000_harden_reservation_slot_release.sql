-- MesaFácil — harden public reservation slot conflicts and release reservation on account close.
...
begin
  ...
  if exists (
    ...
    where existing.tenant_id = target_tenant_id
      ...
      and existing.scheduled_at = reservation_scheduled_at
...
begin
  ...
  with current_reservation as (
    ...
    where r.tenant_id = new.tenant_id
      ...
      -- da mesma mesa no mesmo dia (ex.: almoço fechado às 14h não libera reserva das 20h).
      and r.scheduled_at <= timezone('utc', now()) + interval '1 hour'
