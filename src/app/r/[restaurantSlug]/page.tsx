...
function PublicProductImage({ product, priority = false }: Readonly<{ product: PublicSiteProduct; priority?: boolean }>) {
  ...
  if (imageStyle) {
    return (
      <span
        ...
        role="img"
        className={priority ? 'block aspect-[1.05/1] w-full bg-stone-100 bg-cover bg-center' : 'block aspect-[4/3] w-full bg-stone-100 bg-cover bg-center transition duration-500 group-hover:scale-[1.04]'}
        style={imageStyle}
      />
    );
  }
  ...
  return (
    <div className={priority ? 'flex aspect-[1.05/1] w-full items-center justify-center bg-gradient-to-br from-red-50 via-orange-50 to-amber-100 text-center' : 'flex aspect-[4/3] w-full items-center justify-center bg-gradient-to-br from-red-50 via-orange-50 to-amber-100 text-center'}>
      <div>
...
export default async function PublicRestaurantSitePage({
  ...
  return (
    <main className="min-h-screen bg-[#fffaf5] text-[#241f1c]">
      <section className="relative overflow-hidden bg-[#ff385c] text-white">
        ...
        <div className="relative mx-auto max-w-6xl px-4 pb-16 pt-5 sm:px-5 lg:px-6 lg:pb-20">
          <nav className="flex items-center justify-between rounded-full border border-white/20 bg-white/15 px-4 py-3 text-xs font-black uppercase tracking-[0.18em] shadow-2xl shadow-red-950/10 backdrop-blur">
            <span>MesaFácil</span>
            {site.profile.show_menu ? <a href="#cardapio" className="rounded-full bg-white px-3 py-2 text-[#e00b41]">Ver cardápio</a> : null}
          </nav>

          <div className="mt-10 grid items-end gap-8 lg:grid-cols-[0.98fr_1.02fr]">
            <header className="pb-2">
              <p className="inline-flex rounded-full bg-white/18 px-3 py-1 text-[0.68rem] font-black uppercase tracking-[0.22em] text-white shadow-sm ring-1 ring-white/20">Site oficial do restaurante</p>
              <h1 className="mt-5 max-w-3xl text-5xl font-black leading-[0.94] tracking-[-0.07em] sm:text-7xl lg:text-8xl">{site.profile.display_name}</h1>
              <p className="mt-5 max-w-xl text-lg font-bold leading-8 text-white sm:text-xl">{heroText}</p>
              ...
              <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                {whatsappHref ? (
                  <a href={whatsappHref} className="inline-flex min-h-14 items-center justify-center gap-2 rounded-full bg-[#222222] px-6 py-4 text-base font-black text-white shadow-2xl shadow-red-950/25 transition hover:-translate-y-0.5 hover:bg-black">
                    <span aria-hidden="true" className="grid h-7 w-7 place-items-center rounded-full bg-[#25d366] text-white">✆</span>
                    Chamar no WhatsApp
                  </a>
                ) : primaryContact ? (
                  <a href={primaryContact} className="inline-flex min-h-14 items-center justify-center rounded-full bg-[#222222] px-6 py-4 text-base font-black text-white shadow-2xl shadow-red-950/25 transition hover:-translate-y-0.5 hover:bg-black">Entrar em contato</a>
                ) : null}
                {site.profile.show_menu ? <a href="#cardapio" className="inline-flex min-h-14 items-center justify-center rounded-full bg-white px-6 py-4 text-base font-black text-[#e00b41] shadow-xl shadow-red-950/10 transition hover:-translate-y-0.5">Explorar cardápio</a> : null}
              </div>
            </header>
                ...
                <div className="p-3 sm:p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <h2 className="mt-1 text-2xl font-black tracking-[-0.04em]">{featuredProduct?.name ?? 'Cardápio digital'}</h2>
                    </div>
                    {featuredProduct ? <strong className="rounded-full bg-[#222222] px-3 py-2 text-sm text-white">{money(featuredProduct.price_cents)}</strong> : null}
                  </div>
      ...
      <section className="relative z-10 mx-auto -mt-8 max-w-6xl px-4 sm:px-5 lg:px-6">
      ...
      <section className="mx-auto max-w-6xl px-4 pb-28 pt-10 sm:px-5 lg:px-6">
          ...
          <section id="cardapio" className="space-y-8">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <h2 className="mt-1 text-4xl font-black tracking-[-0.06em] text-[#222222] sm:text-5xl">Escolha pelo visual</h2>
              </div>
                  ...
                  <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                    {category.products.map((product) => (
                      <div key={product.public_code} className="group overflow-hidden rounded-[1.8rem] bg-white shadow-[rgba(0,0,0,0.02)_0px_0px_0px_1px,rgba(0,0,0,0.04)_0px_2px_6px,rgba(0,0,0,0.10)_0px_4px_14px] ring-1 ring-black/5 transition hover:-translate-y-1 hover:shadow-[0_20px_45px_rgba(34,34,34,0.16)]">
                        ...
                        <div className="p-5">
                          <h4 className="text-xl font-black leading-tight tracking-[-0.03em] text-[#222222]">{product.name}</h4>
