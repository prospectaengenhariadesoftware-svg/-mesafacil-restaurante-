  ...
  return (
    <main className="min-h-screen bg-stone-50 px-4 py-8 text-stone-950">
      <section className="mx-auto max-w-4xl space-y-6">
        <header className="rounded-3xl border border-stone-200 bg-gradient-to-br from-white to-red-50 p-6 shadow-2xl shadow-red-100/70">
          <p className="text-sm font-semibold text-red-600">MesaFácil</p>
        ...
        {availableProductsCount === 0 ? (
          ...
          <>
            ...
            {menu.categories.map((category) => (
              <section key={category.name} className="rounded-3xl border border-stone-200 bg-white/95 p-5">
                ...
                {category.products.length === 0 ? (
                  <p className="mt-4 text-sm text-stone-400">Nenhum produto disponível nesta categoria.</p>
                ) : (
                      ...
                      <article key={product.public_code} className="rounded-2xl border border-stone-200 bg-stone-50 p-4">
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex gap-3">
                            {product.image_url ? (
                              <span aria-label={`Imagem de ${product.name}`} role="img" className="h-24 w-24 shrink-0 rounded-2xl border border-stone-200 bg-cover bg-center" style={{ backgroundImage: `url(${product.image_url})` }} />
                            ) : null}
                            <div>
                          ...
                          <p className="shrink-0 rounded-full bg-red-500 px-3 py-1 text-sm font-black text-white">
