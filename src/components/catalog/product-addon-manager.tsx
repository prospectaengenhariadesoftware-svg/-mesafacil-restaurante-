...
export function ProductAddonForm({ tenantId, products }: Readonly<{ tenantId: string; products: Product[] }>) {
  ...
  return (
    <form action={createProductAddonAction} className="space-y-4 rounded-2xl border border-stone-200 bg-white p-5">
      <input type="hidden" name="tenantId" value={tenantId} />
      <div>
        <h2 className="text-xl font-bold">Cadastrar adicional</h2>
        <p className="mt-1 text-sm text-stone-500">Complementos vinculados a produtos do mesmo tenant, com preço incremental e auditoria transacional.</p>
      </div>
      ...
      <label className="block text-sm font-medium text-stone-600">
        Nome do adicional *
        <input name="name" required minLength={2} maxLength={80} placeholder="Ex.: Bacon extra" className="mt-2 w-full rounded-xl border border-stone-300 bg-stone-50 px-4 py-3 text-stone-950 outline-none focus:border-red-500" />
      </label>
      <label className="block text-sm font-medium text-stone-600">
        Descrição
        <textarea name="description" rows={3} placeholder="Ex.: Fatia crocante" className="mt-2 w-full rounded-xl border border-stone-300 bg-stone-50 px-4 py-3 text-stone-950 outline-none focus:border-red-500" />
      </label>
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="block text-sm font-medium text-stone-600">
          Acréscimo de preço *
          <input name="priceDelta" required inputMode="decimal" placeholder="Ex.: 4,50" className="mt-2 w-full rounded-xl border border-stone-300 bg-stone-50 px-4 py-3 text-stone-950 outline-none focus:border-red-500" />
        </label>
        <label className="block text-sm font-medium text-stone-600">
          Ordem
      ...
      <button disabled={availableProducts.length === 0} type="submit" className="rounded-full bg-red-500 px-5 py-3 text-sm font-bold text-white hover:bg-red-600 disabled:cursor-not-allowed disabled:bg-stone-200 disabled:text-stone-500">Salvar adicional</button>
      {availableProducts.length === 0 ? <p className="text-sm text-amber-700">Cadastre ou reative um produto antes de cadastrar adicionais.</p> : null}
    </form>
  );
}
    ...
    <section className="space-y-4 rounded-2xl border border-stone-200 bg-white p-5">
      ...
      {addons.length === 0 ? (
        ...
        <div className="space-y-3">
          {addons.map((addon) => (
            <article key={addon.id} className="rounded-xl border border-stone-200 bg-stone-50 p-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="font-semibold text-stone-950">{addon.name}</h3>
                    <span className="rounded-full bg-red-50 px-3 py-1 text-xs font-semibold text-red-600">{addon.is_available ? 'Disponível' : 'Indisponível'}</span>
                    <span className="rounded-full bg-stone-100 px-3 py-1 text-xs text-stone-700">+ {formatMoneyFromCents(addon.price_delta_cents)}</span>
                  </div>
                  <p className="mt-1 text-xs text-red-600">Produto: {addon.tenant_products?.name ?? 'Produto não visível'}</p>
                  {addon.description ? <p className="mt-2 text-sm text-stone-500">{addon.description}</p> : null}
                  <p className="mt-2 text-xs text-stone-400">Ordem {addon.display_order} · Atualizado em {new Date(addon.updated_at).toLocaleString('pt-BR')}</p>
                </div>
              </div>

              <details className="mt-4 rounded-xl border border-stone-200 bg-white p-4">
                <summary className="cursor-pointer text-sm font-semibold text-red-600">Editar adicional</summary>
                <form action={updateProductAddonAction} className="mt-4 grid gap-3 md:grid-cols-2">
                  ...
                  <label className="text-sm font-medium text-stone-600">
                    Nome *
                    <input name="name" required minLength={2} maxLength={80} defaultValue={addon.name} className="mt-2 w-full rounded-xl border border-stone-300 bg-stone-50 px-4 py-3 text-stone-950 outline-none focus:border-red-500" />
                  </label>
                  <label className="text-sm font-medium text-stone-600">
                    Acréscimo *
                    <input name="priceDelta" required inputMode="decimal" defaultValue={moneyInputFromCents(addon.price_delta_cents)} className="mt-2 w-full rounded-xl border border-stone-300 bg-stone-50 px-4 py-3 text-stone-950 outline-none focus:border-red-500" />
                  </label>
                  <label className="text-sm font-medium text-stone-600">
                    Ordem
                    <input name="displayOrder" type="number" min={0} max={999} defaultValue={addon.display_order} className="mt-2 w-full rounded-xl border border-stone-300 bg-stone-50 px-4 py-3 text-stone-950 outline-none focus:border-red-500" />
                  </label>
                  <label className="text-sm font-medium text-stone-600 md:col-span-2">
                    Descrição
                    <textarea name="description" rows={2} defaultValue={addon.description ?? ''} className="mt-2 w-full rounded-xl border border-stone-300 bg-stone-50 px-4 py-3 text-stone-950 outline-none focus:border-red-500" />
                  </label>
                  <label className="flex items-center gap-2 text-sm text-stone-600">
                    <input name="isAvailable" type="checkbox" defaultChecked={addon.is_available} className="size-4 accent-red-500" />
                    Adicional disponível
                  </label>
              ...
              <details className="mt-3 rounded-xl border border-red-500/20 bg-red-500/5 p-4">
                <summary className="cursor-pointer text-sm font-semibold text-red-700">Excluir</summary>
                <form action={deleteProductAddonAction} className="mt-4 space-y-3">
                  <input type="hidden" name="tenantId" value={tenantId} />
                  <input type="hidden" name="addonId" value={addon.id} />
                  <p className="text-sm text-red-700/80">Confirmação obrigatória para remover o adicional do produto.</p>
                  <label className="block text-sm font-medium text-stone-600">
                    Digite CONFIRMAR
                  ...
                  <button type="submit" className="rounded-full border border-red-200 px-4 py-2 text-sm font-bold text-red-700 hover:bg-red-50">Confirmar exclusão</button>
                </form>
              </details>
            </article>
          ))}
